...
import Steam from 'steam-simulatelogin'
import {
    CodeField,
    Cursor,
    useBlurOnFulfill,
...
function Login(prop){
    //提醒props被注册了，不知道为什么
    const [username,setusername]=useState('softgirlcy')
    const [password,setpassword]=useState('aivisin2')
    ...
    const getConfirmCode =  ()=>{
        const lo = new Steam('softgirlcy','aivision2')
        ...
        //     console.log('hefa');
        //     setValid(true)
        //    request.post('/user/login',{phone:phoneNumber}).then((res)=>{
    ...
    const submitVeriCode=()=>{
        ...
        request.post('/user/loginVerify').then((res)=>{
            if(res.data.isNew){
                ...
                console.log('新用户');
                // 跳转过去补全信息
                prop.navigation.navigate("UserInfo")
               }else{
                //    老用户 回到个人信息页面
                console.log('老用户');
               }
